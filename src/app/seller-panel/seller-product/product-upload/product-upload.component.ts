...
ngOnInit(): void {
    ...
    this.productUploadService.getFormBuilder().subscribe(
      (data: any) => {
        ...
        this.dynamicallykeysAndValidationBuilder(this.formfields);
    ...
    dynamicallykeysAndValidationBuilder(dataReceiver: any[]) {
      dataReceiver.forEach((formKeys) => {
        ...
        if (formKeys.required) 
          validators.push(Validators.required);
  
        if (formKeys.minLength !== undefined && formKeys.minLength !== null && formKeys.minLength !== "") {
          validators.push(Validators.minLength(Number(formKeys.minLength)));
        }
  
        if (formKeys.maxLength !== undefined && formKeys.maxLength !== null && formKeys.maxLength !== "") {
        ...
        if (formKeys.type === 'DROPDOWN') {
          this.filteredDropdowns[formKeys.identifier] =
            this.productForm.get(formKeys.identifier)!.valueChanges.pipe(
              startWith(''),
...
checkPrice() {
  const mrp = Number(this.productForm.get('mrpPrice')?.value);
  const sellingPrice = Number(this.productForm.get('sellingPrice')?.value);

  if (!isNaN(mrp) && !isNaN(sellingPrice) && sellingPrice < mrp) {
    // agar sellingPrice zyada hai MRP se, to error lagao
    this.productForm.get('sellingPrice')?.setErrors({ greater: true });
  } else {
    // warna error hata do
    this.productForm.get('sellingPrice')?.setErrors(null);
  }
}
    ...
    this.hasMultiSelectValues = this.tableRows.length > 0 || selectedValues.length > 0;

    // Remove unselected rows
    for (let i = this.tableRows.length - 1; i >= 0; i--) {
      const row = this.tableRows.at(i) as FormGroup;
      if (row.get('__msId')?.value === field.identifier &&
    ...
    selectedValues.forEach((val) => {
      const exists = this.tableRows.controls.some(ctrl =>
